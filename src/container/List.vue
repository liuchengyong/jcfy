<template>
  <div class="list" ref="list">
   	...
   	<div class="filter" v-if="source == 'list'">
   		<div class="filter-item" v-for="item in currentFilterList" :key="item.type" @click="()=>{isShowFilter=true;filterType = item.type}" :style="{color:item.statu ? '#00ae66': '#999'}">
...
<script>
    ...
    import gps from '../common/gps.js'
  	...
  	export default {
        ...
        source(){ //
          return ['near','list','guanzhu'].includes(this.$route.query.type) ? this.$route.query.type : 'list'
        },
        nullString(){
