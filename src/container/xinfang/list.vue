<template>
  <div class="xinfang-list" ref="list">
    <div class="search">
    ...
    <div class="wrapper">
      <house v-for="(house,index) in list" :key="index" :house="house" :type="type" @handleHosue="handleHosue"></house>
    </div>
    ...
    export default {
      ...
      methods:{
        ...
        handleHosue(type,house){
          if(type == 'detail'){
            this.$router.push({name:'xingfangdetail',query:{id:house.id}})
          }
        },
        goToTop(){
        ...
        getHouseList(page){
          ...
          actions.page_fangwu(data).then(res=>{
            if(res.is_success == 0){
