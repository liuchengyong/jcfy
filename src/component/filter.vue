<template>
  	<div class="component-filter" @click.self="$emit('change','close')" ref="wrapper">
   		<ul class="filter-nav">
   		<li v-for="filter in filterList" :key="filter.type" @click="selectType = filter.type" :style="{color:filter.type == selectType ? '#00ae66': '#333333'}">
   			{{filter.title}}
	  ...
	  <div :class="['wp-area',select.area.id ? 'level3':'level2']" v-if="selectType == 'area'">
			...
			<ul class="wp-area-level wp-area-level2 scrollWrapper">
				<li v-for="area in areaList" :key="area.id" @click="selectArea(area)" :class='{check:select.area.id == area.id}'>{{area.name}}</li>
			</ul>
			<ul class="wp-area-level wp-area-level3 scrollWrapper" v-if="select.area.id">
				<li v-for="area in area2List" :key="area.id" @click="selectArea2(area)" :class='{check:select.area2.id == area.id}'>{{area.name}}</li>
			</ul>
		</div>
		<div class="wp-price" v-if="selectType == 'price'">
			<ul class="wp-price-list scrollWrapper">
				<li v-for="price in priceList" :key="price.id" @click="selectPrice(1,price)" :class="{check:select.price.id == price.id}">{{price.name}}</li>
				<li class="price-self">
					自定义价格
		...
		<div class="wp-fangxing" v-if="selectType == 'fangxing'">
			<ul class="wp-fangxing-list scrollWrapper">
				<li v-for="fangxing in fangXingList" :key="fangxing.id" @click="selectArray('fangxing',fangxing)">
		...
		<div class="wp-other" v-if="selectType == 'other'">
			<ul class="wp-other-list scrollWrapper">
				<li class="wo-item">
					...
					<ul>
						<li :class="{check:select.chaoxiang.id == item.id}" v-for="item in chaoxiangList" :key="item.id" @click="select.chaoxiang = item">{{item.name}}</li>
					</ul>
				</li>
				<li class="wo-item">
					<span class="title">面积</span>
					<ul>
						<li :class="{check:select.mianji.id == item.id}"  v-for="item in mianjiList" :key="item.id" @click="select.mianji = item">{{item.name}}</li>
					</ul>
				</li>
				<!-- <li class="wo-item">
				...
				<li class="wo-item">
					...
					<ul>
						<li v-for="item in loulingList" :key="item.id" :class="{check:select.louling.id == item.id}" @click="select.louling = item">{{item.name}}</li>
					</ul>
				</li>
				<li class="wo-item">
					<span class="title">装修</span>
					<ul>
						<li v-for="item in zhuangxiuList" :key="item.id" :class="{check:select.zhuangxiu.id == item.id}"  @click="select.zhuangxiu = item">{{item.name}}</li>
					</ul>
				</li>
				<li class="wo-item">
					<span class="title">电梯</span>
					<ul>
						<li v-for="item in diantiList" :key="item.id" :class="{check:select.dianti.id == item.id}"  @click="select.dianti = item">{{item.name}}</li>
					</ul>
				</li>
				<li class="wo-item">
					<span class="title">用途</span>
					<ul>
						<li v-for="item in yongtuList" :key="item.id" :class="{check:select.yongtu.id == item.id}"  @click="select.yongtu = item">{{item.name}}</li>
					</ul>
				</li>
        <li class="wo-item" v-if="isSystem()">
          <span class="title">状态</span>
          <ul>
            <li v-for="item in statusList" :key="item.id" :class="{check:select.status.id == item.id}"  @click="select.status = item">{{item.name}}</li>
          </ul>
        </li>
        <li class="wo-item" >
          <span class="title">VR</span>
          <ul>
            <li v-for="item in vrList" :key="item.id" :class="{check:select.vr.id == item.id}"  @click="select.vr = item">{{item.name}}</li>
          </ul>
        </li>
				<!-- <li class="wo-item">
